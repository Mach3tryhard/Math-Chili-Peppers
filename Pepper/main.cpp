...
double F(double x) {
    double rez = 255 * pow(exp(1), pow(-exp(1), -1000 * x));
...
double W(double x, double y) {
    double rez = 0;
    for (int s = 1; s <= 40; s++) {
        double frac1 = 11.0 / 10.0;
        double frac2 = 19.0 / 20.0;
...
double K(double v, double s, double x, double y) {
    double frac1 = 7.0 / 10.0;
    double frac2 = 1.0 / 2.0;
    double frac3 = 97.0 / 100.0;
    double frac4 = 1.0 / 50.0;
    double frac5 = 23.0 / 50.0;
    double frac6 = 7.0 / 20.0;
...
double L(double v, double s, double x, double y) {
    ...
    double frac2 = 4.0 / 5.0;
    double frac3 = 7.0 / 10.0;
...
double N(double s, double x, double y) {
    double frac1 = 1.0 / 2.0;
    double franc2 = 4.0 / 5.0;
    double franc3 = 96.0 / 25.0;

    double a = abs(cos(P(s, x, y) - pow(sin(frac1 * Q(s, x, y) + pi / 4), 6)));
    double rez = pow(exp(1), pow(-exp(1), -6 * pow(a, 10 + 9 * sin(Q(s, x, y))) * pow(abs(cos(Q(s, x, y))), franc2) + franc3));
    return rez;
}

double B(double s, double x, double y) {
    double a = 1.0 / 2.0 * Q(s, x, y) + pi / 4 + 1.0 / 20.0 * pow(cos(10 * P(s, x, y) + cos(3 * s)), 4);
    double rez = pow(exp(1), pow(-exp(1), -100 * (pow(cos(a), 20) - 1.0 / 10.0)));
    return rez;
}

double C(double s, double v, double x, double y) {
    double a = abs(cos(P(s, x, y)) - pow(sin(1.0 / 2.0 * Q(s, x, y) + pi / 2), 6));
    double b = 10 + 9 * sin(Q(s, x, y));
    double c = pow(abs(Q(s, x, y)), 4.0 / 5.0);
    double d = (30 + 10 * cos(4 * s)) * (Q(s, x, y) + 3.0 / 100.0 * cos(5 * P(s, x, y))) + 2 * cos(17 * Q(s, x, y) + P(s, x, y) + 2) + cos(27 * Q(s, x, y)) - 17 * P(s, x, y) + 2 * cos(5 * s);
    double rez = pow(exp(1), pow(-exp(1), -v * pow(a, b) * c * (1.0 / 10.0 * cos(d) + 9.0 / 10.0) + 97.0 / 100.0 * v - 3.0 / 100.0 * v * W(x, y)));
    return rez;
}
...
double A(double v, double s, double x, double y) {
    double rez = pow(10, -1) * (1 - B(s, x, y)) * (4 * v * v - 16 * v + 16 + pow(-1, s) * (11 * v - 5 * v * v - 2) + (5 * v - 3 * v * v + 2) * cos((7 + v) * s) + pow(20, -1) * (40 - s) * (6 * C(20, s, x, y) + 5 * C(5, s, x, y) - 2 * R(s, x, y)));
...
void Calculate(HDC hdc) {
    for (int m = 1; m <= 2000; m++) {
        for (int n = 1; n <= 1200; n++) {
            double hx0 = H(0, (m - 1000) / 600.0, (601 - n) / 600.0);
            double hx1 = H(1, (m - 1000) / 600.0, (601 - n) / 600.0);
            double hx2 = H(2, (m - 1000) / 600.0, (601 - n) / 600.0);
            int r = static_cast<int>(F(hx0));
            int g = static_cast<int>(F(hx1));
            int b = static_cast<int>(F(hx2));
            SetPixel(hdc, n, m, RGB(r, g, b));
        }
    }
}
